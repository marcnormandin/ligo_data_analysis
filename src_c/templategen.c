...
int templategen(const char* Id, const double chirp_time0_input, const double chirp_time1_5_input,
  ...
  const double calculated_reduced_mass = (1.0/(16.0*gsl_pow_2(f_low)))*pow(5.0/(4.0*gsl_pow_4(M_PI)*chirp_time0*gsl_pow_2(chirp_time1_5)), 1.0/3.0)/(G/gsl_pow_3(c));
  //Calculated total mass  using  chirp_time0 and chirp_time1_5                                                                       
  const double calculated_total_mass = (5.0/(32.0*f_low))*(chirp_time1_5/(gsl_pow_2(M_PI)*chirp_time0)) / (G/gsl_pow_3(c));
    
  const double s_mass_ratio_cal = calculated_reduced_mass / calculated_total_mass;
  const double multi_fac_cal = f_low * M_PI * G * calculated_total_mass/(gsl_pow_3(c));
   
  const double chirp_time1 = (5.0/(192.0*M_PI)) / f_low / multi_fac_cal / s_mass_ratio_cal * ((743.0/336.0)+((11.0/4.0)*s_mass_ratio_cal));
  const double chirp_time2 =   (5.0/(128.0*M_PI)) / f_low / pow(multi_fac_cal, 1.0/3.0) / s_mass_ratio_cal * ((3058673.0/1016064.0)+((5429.0/1008.0)*s_mass_ratio_cal)+ ((617.0/144.0)*(gsl_pow_2(s_mass_ratio_cal))));
   ...
   //SPA_0 = ones(1, len_f);       SPA_90= ones(1,len_f);
    ...
    for (int jj=0; jj < len_f; ++jj) {
        ...
        if (f_low < f[jj] && f[jj] <= f_high) {
            ...
            const double amp_SPA =  (1.0/g)*Amp_fact1*Amp_fact2*(pow(f[jj], (-7/6)));
            const double f_fac = f[jj] / f_low;
            
            const double phase_SPA = 2.0*M_PI*f[jj]*(tc_template - delay) - 2.0*coalesce_phase - M_PI/4.0 +
                            + (2.0*M_PI*f_low) * (
                                             ( 3.0*chirp_time0 *( pow(f_fac, -5.0/3.0) )/5.0  )
                                            +( chirp_time1*pow(f_fac, -5.0/3.0) )
                                            -( 3.0*chirp_time1_5* pow( f_fac, -2.0/3.0 ) / 2.0 )
                                            +( 3.0*chirp_time2* pow(f_fac, -1.0/3.0) )
                                         );
