...
/*void outerproduct(gsl_vector* x, gsl_vector* y, gsl_matrix* product) {
    gsl_matrix_set_zero(product);
    gsl_blas_dger(1.0, x, y, product);
}*/
...
int antennapattern(double declination, double right_ascention, double polarization_angle, 
    const char* Intefe_ID,
    double *u, double *v, double *F_Plus, double *F_Cross) 
{
    ...
    init_vector(n_hat,
                    gsl_sf_cos(right_ascention)*gsl_sf_cos(declination),
                    gsl_sf_sin(right_ascention)*gsl_sf_cos(declination), 
                    gsl_sf_sin(declination)
                );

    gsl_vector* ex_i = gsl_vector_alloc(3);
    init_vector(ex_i,
                    gsl_sf_sin(right_ascention), 
                    -gsl_sf_cos(right_ascention), 
                    0
                );

    gsl_vector* ey_j = gsl_vector_alloc(3);
    init_vector(ey_j, 
                    -gsl_sf_cos(right_ascention)*gsl_sf_sin(declination),
                    -gsl_sf_sin(right_ascention)*gsl_sf_sin(declination), 
                    gsl_sf_cos(declination)
                );
    ...
    //epsilon_plus = (transpose(ex_i)*ex_i)-(transpose(ey_j)*ey_j);
    ...
    //epsilon_cross = (transpose(ex_i)*ey_j)+(transpose(ey_j)*ex_i);


    ////  Detector Tensor Calculations --------------------------------------------------
    // Make sure that the unit vectors are in 1x3 form
    //Cordnt.U = transpose(cordnt.X(:)); 
    //cordnt.V = transpose(cordnt.Y(:));
    ...
    //D = 0.5*((transpose(Cordnt.U)*Cordnt.U)-(transpose(cordnt.V)*cordnt.V));
    //----------------------------------------------------------------------------------
    ...
    gsl_blas_dgemm(CblasNoTrans, CblasNoTrans, 1.0, D, epsilon_cross, 0.0, temp);
    trace(temp, v);
    ...
    //F = wp_matrix * transpose([u v]);   
    //*F_Plus  = F(1);     
    //*F_Cross = F(2);
    *F_Plus = gsl_matrix_get(wp_matrix, 0, 0) * *u + gsl_matrix_get(wp_matrix, 0, 1) * *v;
    *F_Cross = gsl_matrix_get(wp_matrix, 1, 0) * *u + gsl_matrix_get(wp_matrix, 1, 1) * *v;
