...
c=299792458;            G=6.67408e-11;              solar_mass = 1.9891e30;

%  Signal Paramaeters %
    right_ascension     =  -2.14;     %right_ascension of the signal
...
% Conversion of binary masses to the solar mass.
     m1=m1*solar_mass;   m2=m2*solar_mass;
...
% tchirp         (Time vector for the chirp)(time_of_arrival:0.01:tc)
    ...
    %Amp_fact1 = sqrt(5/24)*(1/(pi^(2/3)*oneMpc));
    ...
    s_mass_ratio = reduced_mass/total_mass; 
    multi_fac = f_low * pi * G * total_mass/(c^3);
 
    %multi_fac = (5 / (256*s_mass_ratio))* (G*total_mass/(c^3));
    %v_low = (G*total_mass * pi * f_low / (c^3))^(1/3);
    
    % For 1.4xSolar_mass binaries
    % chirp_time0 = 24.8499; chirp_time1 = 1.3861; 
    % chirp_time1_5 = 0.8662; chirp_time2 = 0.0479; 
    
    chirp_time0 = (5/(256*pi)) * f_low^-1 * multi_fac^(-5/3) * s_mass_ratio^-1;
    
    %chirp_time1 = (5/(192*pi)) * f_low^-1 * multi_fac^(-1) * s_mass_ratio^-1 * ...
    %                                              ((743/336)+((11/4)*s_mass_ratio));
                                              
    chirp_time1_5 = (1/8) * f_low^-1 * multi_fac^(-2/3) * s_mass_ratio^-1;
    
    %chirp_time2 =   (5/(128*pi)) * f_low^-1 * multi_fac^(-1/3) * s_mass_ratio^-1 * ...
...
for id = 1:1:length(detId)
    ...
    for jj=1:1:length(f)
        ...
        if (f_low< f(jj)&& f(jj)<=f_high)
            ...
            %v(jj) = (G*total_mass * pi * f(jj) / (c^3))^(1/3);
            amp_2pn(jj)=  (1/g)*Amp_fact1*Amp_fact2*(f(jj)^(-7/6));
            f_fac(jj) = f(jj)/f_low;
            
            phase_2pn(jj) = 2*pi*f(jj)*(tc - delay(id)) - 2*coalesce_phase - pi/4 + ...
                            + (2*pi*f_low) * (...
